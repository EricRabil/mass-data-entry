...
export namespace IOKit {
    ...
    export async function writeEntries({ structureID }: DataStructure, entries: Entry[]): Promise<void> {
        const name = `${structureID}-entry-${uuid.v1()}`;
        
        await fs.mkdirp(ENTRIES_PATH)
        await fs.writeJSON(path.resolve(ENTRIES_PATH, name), { structureID, entries });
    }
}